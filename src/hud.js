export default class Hud {
  constructor(ctx, store, ratioX=1, ratioY=1) {
    this.ctx = ctx;
    this.store = store;
    this.width = this.store.cannon.width * 0.65 * ratioX;
    this.height = this.store.cannon.height * 0.65 * ratioY;
    this.livesMargin = 20;
    this.font = `${12 * ratioY}px sans-serif`;

    this.x = this.store.marginLeft;
    this.y = this.store.ctxHeight - this.store.marginBottom / 2 - this.height;
  ...
  drawSeparateLine() {
    ...
    this.ctx.fillRect(0, this.store.ctxHeight - this.store.marginBottom, this.store.ctxWidth, 3);
  }

  drawLives() {
     let x = this.store.marginLeft;
     const y = this.store.ctxHeight - this.store.marginBottom / 2 - this.height / 2;

    for (let i = 0; i < this.store.cannon.lives; i++) {
      this.ctx.drawImage(this.sprite, x, y, this.width, this.height);
      x += this.livesMargin;
    }
  }
}
